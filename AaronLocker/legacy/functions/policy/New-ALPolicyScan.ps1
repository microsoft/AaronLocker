...
#>
	...
	<#
		...
		.PARAMETER Paths
			The paths to replace the username in.
		
		.PARAMETER ForUsername
			The username to insert into the path.
	#>
		[CmdletBinding()]
		param (
		...
		process
		{
			...
			{
				if ($path -match '\\\*$') { $path = [System.IO.Path]::GetFullPath($path.TrimEnd("*")) + "*" }
				else { $path = [System.IO.Path]::GetFullPath($path) }
				$path -replace "\\$env:USERNAME\\", "\$ForUsername\" -replace "\\$($env:USERNAME)$", "\$ForUsername"
			}
		}
	}
	...
	if (-not (Test-Path -Path $customizationInputsDir)) { mkdir $customizationInputsDir | Out-Null }
	if (-not (Test-Path -Path $mergeRulesDynamicDir)) { mkdir $mergeRulesDynamicDir | Out-Null }
	if (-not (Test-Path -Path $mergeRulesStaticDir)) { mkdir $mergeRulesStaticDir | Out-Null }
	if (-not (Test-Path -Path $outputsDir)) { mkdir $outputsDir | Out-Null }
	if (-not (Test-Path -Path $supportDir)) { mkdir $supportDir | Out-Null }
	if (-not (Test-Path -Path $scanResultsDir)) { mkdir $scanResultsDir | Out-Null }
	...
	if (-not ((Test-Path -Path $windirTxt) -and (Test-Path -Path $PfTxt) -and (Test-Path -Path $Pf86Txt)))
	{
		$Rescan = $true
	}
	
	#region Process Windir and ProgramFiles directories.
	...
	if ($Rescan)
	{
		...
		Search-WritableDirectory -RootDirectory $env:windir -ShowGrantees -OutputXML -KnownAdmins $knownAdmins | Out-File -Encoding ASCII $windirFullXml
		Write-Host "Enumerating writable directories in $env:ProgramFiles" -ForegroundColor Cyan
		Search-WritableDirectory -RootDirectory $env:ProgramFiles -ShowGrantees -OutputXML -KnownAdmins $knownAdmins | Out-File -Encoding ASCII $PfFullXml
		...
		if ($null -ne ${env:ProgramFiles(x86)})
		{
			...
			Search-WritableDirectory -RootDirectory ${env:ProgramFiles(x86)} -ShowGrantees -OutputXML -KnownAdmins $knownAdmins | Out-File -Encoding ASCII $Pf86FullXml
		}
		else
		{
		...
		function RemoveRedundantLines([String]$fnameFullXml)
		{
			...
			if ($null -ne $x)
			{
				...
				$x.root.dir.name | Sort-Object | ForEach-Object {
	...
	#endregion Process Windir and ProgramFiles directories.
	
	#region Capture data for Exe files to blacklist if needed
	if ($Rescan -or -not (Test-Path -Path $ExeBlacklistData))
	{
		...
		$pubCollection = @{ }
		$exeFilesToBlacklist | ForEach-Object {
	...
	#endregion Capture data for Exe files to blacklist if needed
	
	#region Validate that scan-result files were created
	if (-not ((Test-Path -Path $windirTxt) -and (Test-Path -Path $PfTxt) -and (Test-Path -Path $Pf86Txt)))
	{
		$errMsg = @"
One or more of the following files is missing:
	$windirTxt
	$PfTxt
	$Pf86Txt
"@
	...
	if (-not (Test-Path -Path $ExeBlacklistData))
	{
		$errMsg = @"
...
"@
	...
	#endregion Validate that scan-result files were created
	...
	#region Incorporate data for EXE files to blacklist under Windir
	...
	$csvExeBlacklistData | ForEach-Object {
	...
	#endregion Incorporate data for EXE files to blacklist under Windir
	...
	#region Create rules for trusted publishers
...
"@
	...
	#endregion Create rules for trusted publishers
	
	#region Create custom hash rules
...
"@
	...
	#endregion Create custom hash rules
	
	#region Rules for files in user-writable directories
	# --------------------------------------------------------------------------------
	...
	else
	{
		...
		Get-ALRulePath | ForEach-Object {
			$label = $_.label
			if ($ForUser)
			{
				$paths = Rename-Paths -Paths $_.paths -ForUsername $ForUser
			}
			else
			{
				$paths = $_.paths
			...
			$paramConvertToALPolicy = @{
				Path    = $paths
				Recurse = (-not ([bool]$_.noRecurse))
				EnforceMinimumVersion = ([bool]$_.enforceMinVersion)
				RuleNamePrefix = $label
			}
			
			$outfile = Join-Path $mergeRulesDynamicDir "$($label)Rules.xml"
			# If it already exists, create a name that doesn't exist yet
			$ixOutfile = 2
			while (Test-Path -Path $outfile)
			{
				$outfile = Join-Path $mergeRulesDynamicDir "$($label) ($($ixOutfile)) Rules.xml"
				$ixOutfile++
			}
			Write-Host ("Scanning $label`:", $paths) -Separator "`n`t" -ForegroundColor Cyan
			
			SaveAppLockerPolicyAsUnicodeXml -ALPolicy (ConvertTo-ALPolicy @paramConvertToALPolicy | Join-ALPolicy) -xmlFilename $outfile
		}
	}
	#endregion Rules for files in user-writable directories
	...
	Get-ChildItem $mergeRulesDynamicDir\*.xml, $mergeRulesStaticDir\*.xml | ForEach-Object {
		$policyFileToMerge = $_
	...
	if ($Excel)
	{
		Export-ALPolicyToExcel -AppLockerXML $rulesFileEnforceNew -SaveWorkbook
		Export-ALPolicyToExcel -AppLockerXML $rulesFileAuditNew -SaveWorkbook
	}
	#endregion Generate final outputs
}